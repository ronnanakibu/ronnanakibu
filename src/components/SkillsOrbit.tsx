  ...
  return (
    <div className="w-full select-none">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center justify-items-center w-full">
        ...
        {/* Right: Active Orbit Details Display Box */}
        <div className="w-full max-w-md lg:max-w-none bg-surface-dark/60 border border-white/5 rounded-2xl p-6 md:p-8 backdrop-blur-md relative overflow-hidden transition-all duration-300 self-stretch flex flex-col justify-center min-h-[300px]">
          <div className="absolute top-0 left-0 w-1.5 h-full bg-gradient-to-b from-accent-primary to-accent-secondary" />
          
          {activeData ? (
            <div>
              <h4 className="text-text-main font-bold tracking-wide font-sans text-[24px] mb-2 flex items-center space-x-2">
                <span>{activeData.name}</span>
              </h4>
              <p className="text-[14px] text-text-muted font-mono mb-6">Core stack & focus tools:</p>
              <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                {activeData.tools.map((t, idx) => (
                  <div key={idx} className="flex items-center space-x-2 text-[16px] text-text-main/90 font-mono">
                    <span className="w-2 h-2 rounded-full bg-accent-secondary/80 shrink-0" />
                    <span>{t}</span>
                  </div>
                ))}
              </div>
            ...
            <p className="text-center text-text-muted text-[16px] font-mono py-8">Hover categories to explore Rony's skillset.</p>
          )}
        </div>
      </div>
    </div>
